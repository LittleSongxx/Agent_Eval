...
const METRIC_HELP: Record<string, MetricHelpInfo> = {
  ...
  contextual_relevancy: {
    ...
    category: '检索质量',
    compareSubject: 'retrieved_contexts（检索到的上下文）  vs  user_input（用户问题）',
    compareMethod: '1. 平台原生 Judge 阅读用户问题和检索上下文\n2. 判断每段上下文是否直接有助于回答该问题\n3. 无关、只沾边、过泛或噪声片段会扣分\n4. reason 会指出主要相关片段和噪声来源',
    scoreRange: '0.0 ~ 1.0（1.0 表示检索上下文整体高度相关）',
    requiredFields: ['user_input', 'retrieved_contexts'],
    example: '用户问退货政策，检索结果却主要是会员积分和物流时效 → 上下文相关性会低。',
  },
  answer_relevancy: {
  ...
  task_completion: {
    ...
    category: 'Agent 任务',
    compareSubject: '完整 Agent 轨迹  vs  reference（期望任务结果）',
    compareMethod: '1. 平台原生 Judge 阅读用户请求、工具调用、工具返回和最终回复\n2. 判断任务闭环是否完成，而不是只看回复是否像完成了\n3. 按完成程度给出 0~1 分数\n4. reason 会说明任务达成、部分达成或失败的证据',
    scoreRange: '0.0 ~ 1.0（越高表示任务闭环越完整）',
    requiredFields: ['user_input（conversation格式）', 'reference'],
    example: '用户要求查订单并告知物流，Agent 查到订单且给出快递单号 → 高分；只说“我会查询”但没有结果 → 低分。',
  },
  agent_goal_accuracy: {
  ...
  argument_correctness: {
    ...
    category: 'Agent 工具',
    compareSubject: '实际工具参数  vs  reference_tool_calls 中的期望参数',
    compareMethod: '1. 后端先按工具名称匹配实际调用和期望调用\n2. 对同名工具逐项比较参数 key 和 value\n3. 分数 = 每个期望调用的参数匹配度平均值\n4. 这是确定性代码指标，不依赖 LLM',
    scoreRange: '0.0 ~ 1.0（1.0 表示工具参数全部正确）',
    requiredFields: ['user_input（conversation格式）', 'reference_tool_calls'],
    example: '期望 query_order(order_id="001")，实际 query_order(order_id="002") → 工具选对但参数错误，参数正确性低。',
  },
  step_efficiency: {
    name: 'step_efficiency',
    displayName: '步骤效率 (Step Efficiency)',
    category: 'Agent 过程',
    compareSubject: '实际工具步骤数  vs  reference_tool_calls 的期望步骤数',
    compareMethod: '1. 后端统计实际 tool_calls 数量和期望工具调用数量\n2. 数量一致给满分\n3. 多调、漏调都会按比例扣分\n4. 这是确定性代码指标，适合发现重复调用和绕路执行',
    scoreRange: '0.0 ~ 1.0（1.0 表示步骤数量与期望完全一致）',
    requiredFields: ['user_input（conversation格式）', 'reference_tool_calls'],
    example: '期望 2 次工具调用，实际调用 4 次，其中有重复查询 → Step Efficiency = 0.5。',
  },
  topic_adherence: {
  ...
  turn_relevancy: {
    ...
    category: '多轮对话',
    compareSubject: '每轮 AI 回复  vs  当前轮用户输入',
    compareMethod: '1. 平台原生 Judge 按轮次阅读对话\n2. 判断 AI 是否回应当前用户问题和上下文追问\n3. 忽略问题、答非所问或错误沿用旧上下文会扣分\n4. reason 会指出问题轮次',
    scoreRange: '0.0 ~ 1.0（越高表示每轮回复越贴合当前轮用户意图）',
    requiredFields: ['user_input（conversation格式）'],
    example: '用户追问“那退款多久到账”，AI 却继续推荐商品 → 轮次相关性扣分。',
  },
  conversation_completeness: {
    name: 'conversation_completeness',
    displayName: '对话完成度 (Conversation Completeness)',
    category: '多轮对话',
    compareSubject: '完整对话  vs  reference（期望对话结果）',
    compareMethod: '1. 平台原生 Judge 阅读整段对话\n2. 判断是否满足用户需求并完成 reference 描述的结果\n3. 没有收束问题、遗漏关键步骤或没有给出可执行结论会扣分',
    scoreRange: '0.0 ~ 1.0（越高表示整段对话越完整地解决了用户问题）',
    requiredFields: ['user_input（conversation格式）', 'reference'],
    example: '用户咨询退货，AI 说明政策、确认订单并提交申请 → 高分；只解释政策没有推进处理 → 中低分。',
  },
  knowledge_retention: {
    name: 'knowledge_retention',
    displayName: '上下文记忆 (Knowledge Retention)',
    category: '多轮对话',
    compareSubject: '后续 AI 回复  vs  用户前面给出的事实/约束',
    compareMethod: '1. 平台原生 Judge 提取用户已给出的事实、偏好、订单号、时间等\n2. 检查后续回复是否正确保留和使用这些信息\n3. 遗忘、混淆或自相矛盾会扣分',
    scoreRange: '0.0 ~ 1.0（越高表示上下文记忆越稳定）',
    requiredFields: ['user_input（conversation格式）'],
    example: '用户前面说订单号是 ORD-123，后面 AI 查成 ORD-321 → 上下文记忆扣分。',
  },
  role_adherence: {
    name: 'role_adherence',
    displayName: '角色遵守度 (Role Adherence)',
    category: '多轮对话',
    compareSubject: '完整对话  vs  reference_role（指定角色）',
    compareMethod: '1. 平台原生 Judge 阅读 reference_role 和完整对话\n2. 判断 AI 是否遵守角色、职责边界和语气要求\n3. 越权承诺、角色漂移、语气不合适会扣分',
    scoreRange: '0.0 ~ 1.0（越高表示角色越稳定）',
    requiredFields: ['user_input（conversation格式）', 'reference_role'],
    example: '客服助手承诺“我已替你付款下单”超出职责边界 → 角色遵守度扣分。',
  },
  turn_faithfulness: {
    name: 'turn_faithfulness',
    displayName: '轮次忠实度 (Turn Faithfulness)',
    category: '多轮可信度',
    compareSubject: '多轮 AI 回复  vs  retrieved_contexts（检索上下文）',
    compareMethod: '1. 平台原生 Judge 阅读多轮回复和检索上下文\n2. 判断每轮事实性回复是否有上下文支撑\n3. 跨轮幻觉、与资料冲突或无依据承诺会扣分',
    scoreRange: '0.0 ~ 1.0（越高表示多轮事实回复越有依据）',
    requiredFields: ['user_input（conversation格式）', 'retrieved_contexts'],
    example: '资料只说 7 天可退，AI 后续说 30 天可退 → 轮次忠实度扣分。',
  },
  harmfulness: {
    name: 'harmfulness',
