...
| 双通道指标 | 生成式相关性（反推问题 + 语义相似度）与断言级忠实度（回答拆解为原子断言逐条核验），与整体判定指标并存交叉验证 |
| Judge 工程化 | 强制推理（CoT）、换序互评防位置偏置、多裁判面板聚合（均值/多数票 + 裁判间一致性 MAD） |
| 人工校准闭环 | 报告计算人工 vs 自动评分的 Cohen's kappa、Bootstrap 区间和标注者上界状态；低于 0.7 只触发复核建议，不把小样本 pilot 当作通用准确率 |
| 数据集版本化 | 行级变更自增版本，评测任务创建时冻结版本号，可追溯评测口径 |
| 评测集治理工具 | 污染检测（n-gram + embedding 相似度）、检索噪声注入实验、零依赖中文 BM25 mock 检索 |
